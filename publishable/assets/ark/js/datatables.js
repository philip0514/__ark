...
var Datatables = function(){
	...
	var error = function(){
		$('.processing-alert').removeClass('alert-warning').addClass('alert-danger').html('<table class="w-100 h-100"><tr><td class="align-middle">抱歉，出了點差錯...</td></tr></table>');
	};
	...
	var init_complete = function(){
		...
		if(config.action.export){
			...
			item_total = 
				'<optgroup label="全部">'+
					'<option data-icon="fa fa-cloud-download" value="9"> 匯出</option>'+
				'</optgroup>';
		}
		...
		if(item_single){
			...
			'<div class="col-md-6 xs-p-b-10">'+
				'<div class="input-group select-group">'+
					'<select class="select-action show-tick" data-style="btn-primary">'+
						'<optgroup label="功能操作">'+
							item_single+
						'</optgroup>'+
						item_total+
		...
		//欄位顯示
		item_single = '';
		$.each(config.visible, function(index, value){
			item_single += '<option value="'+value.value+'" '+(value.selected?'selected':'')+'>'+value.name+'</option>';
		});
		...
		//整合操作與欄位
		$('.data-action').html('<div class="row">'+content+'</div>');
		...
		$('.btn-search').click(function(){
			...
			$('.btn-filter-delete').each(function(){
				var id = $(this).data('id');
			...
			datatable.search(JSON.stringify(data)).draw();
			*/
			datatable.search($('#search').val()).draw();

		});
		//$('.btn-search').trigger('click').html('1');
		...
		/*
		//autocomplete
		if(config.action.autocomplete){
			var search = new Bloodhound({
				datumTokenizer: Bloodhound.tokenizers.whitespace,
				queryTokenizer: Bloodhound.tokenizers.whitespace,
				limit: 10,
				remote: {
					url: config.ajax_autocomplete+'?q=%QUERY',
					wildcard: '%QUERY'
				}
			});
	...
	var row_reorder = function(){
		datatable.on('row-reorder', function(e, setting, detail){
			var page_current = datatable.page.info().page;
			var page_length = datatable.page.info().length;
			var data = [];
			
			for(var i=0, ien=setting.length ; i<ien ; i++){
				data.push([$(setting[i].node).find('.input-id').val(), (setting[i].newPosition+page_current*page_length)]);
			}
			if(data.length){
				$.ajax({
					url: config.path.rowReorder,
					type: 'POST',
					data: {data: data},
					error: function(xhr, textStatus, errorThrown){
						console.log(xhr.status+' '+errorThrown);
	...
	var col_reorder = function(){
		datatable.on('column-reorder', function(e, setting, detail){
			$.ajax({
				url: config.path.columnReorder,
				type: 'POST',
				data: {
					from: detail.from,
					to: detail.to
	...
	var filter = {
		field: function(){
			...
			$('.btn-filter-delete').parent('li').remove();
			$('.filter input').each(function(){
				...
				if(value){
					var filter = $('<li>').append(
						$('<a>', 
						  {class: 'btn-filter-delete', href:'javascript:;', 'data-id':id, 'data-value':value}
						 ).append(
							$(this).attr('title')+'： '+$(this).val(),
							$('<i>', {class:'fas fa-times-circle'})
						)
					);
					$('.btn-filter-clear').parent('li').before(filter);
				}
			...
			$('.filter select').each(function(){
				...
				if(value && value!="0"){
					...
					var filter = $('<li>').append(
						$('<a>', 
						  {class: 'btn btn-primary btn-filter-delete', href:'javascript:;', 'data-id':id, 'data-value':value}
						 ).append(
							title+'： '+text,
							$('<i>', {class:'fas fa-times-circle'})
						)
		...
		delete: function(){
			...
			$('.btn-filter-clear').unbind('click');
			$('.btn-filter-clear').click(function(){
				$('.btn-filter-delete').each(function(){
					var id = $(this).data('id');
					var value = $(this).data('value');
					var input = $('.filter-content').find('#'+id);
			...
			$('.btn-filter-delete').unbind('click');
	...
	var init = function(){
		...
		datatable = $('#datatable').DataTable({
			...
			"colReorder": config.colReorder,
			"rowReorder": config.rowReorder,
			...
			"serverSide": true,
            "ajax": {
                'url': config.path.datatable,
                'method' : 'get',
                'data': function(data){
					/*
					$filter = $('.filter');
					$.each($filter.serializeArray(), function(index, parameter) {
						if(parameter.value != '') data[parameter.name] = parameter.value;
					});
					*/
			...
			"rowCallback": function(row, data, displayIndex, displayIndexFull){
				...
				if(data.deleted_at != null){
					displayClass = "bg-light text-danger font-italic font-deleted";
				}
				$(row).addClass(displayClass);

				if(data.sortable){
					$(row).find('.fa-sort').parent('td').addClass('sortable');
				}else{
					$(row).find('.fa-sort').parent('td').addClass('unsortable');
				}
			...
			"language": {
				...
				"processing": '<div class="processing-alert alert alert-info"><table class="w-100 h-100"><tr><td class="align-middle">讀取中...</td></tr></table></div>'
			},
